...
public class OVRHapticsClip
{
        ...
        for (int i = 0; i < a.Count || i < b.Count; i++)
        {
            if (OVRHaptics.Config.SampleSizeInBytes == 1)
            {
                ...
                if ((i < a.Count) && (i < b.Count))
                    sample = (byte)(Mathf.Clamp(a.Samples[i] + b.Samples[i], 0,
    ...
    private void InitializeFromAudioFloatTrack(float[] sourceData, double sourceFrequency, int sourceChannelCount,
        int sourceChannel)
    {
        ...
        while (i < length)
        {
            if (OVRHaptics.Config.SampleSizeInBytes == 1)
            {
                WriteSample((byte)(Mathf.Clamp01(Mathf.Abs(sourceData[i])) *
                                   System.Byte.MaxValue)); // TODO support multi-byte samples
            }
